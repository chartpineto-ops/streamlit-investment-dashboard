...
EARNINGS_FILTER_DEFAULTS = {
    ...
    "earnings_search_query": "",
    "earnings_stocktwits_filter": False,
    "earnings_stocktwits_min_mentions": 0,
...
def parse_numeric_value(value: object) -> float | None:
    ...
    if not text or text.lower() in {"n/a", "na", "none", "null", "undefined", "--"}:
        return None
    negative_parentheses = bool(re.search(r"\(\s*\$?\s*\d", text))
    cleaned = (
        text.replace("−", "-")
        .replace("$", "")
        .replace(",", "")
...
def inject_css() -> None:
    st.markdown(
        ...
        <style>
            ...
            .earnings-hidden-summary {
                background: rgba(230, 211, 111, 0.08);
                border: 1px solid rgba(230, 211, 111, 0.22);
                border-radius: 7px;
                color: var(--term-muted);
                font-size: 0.56rem;
                font-weight: 850;
                margin: 0.4rem 0.48rem 0.48rem 0.48rem;
...
def render_company_options_metrics(snapshot: dict) -> None:
    ...
    render_metric_strip(
        [
            ...
            {"label": "Options Expiry Used", "value": expiry_text, "context": f"{snapshot.get('Options Days To Expiry') or 'N/A'} days"},
...
def format_signed_eps(value: object) -> str:
    number = coerce_float(value)
    if number is None:
        return "N/A"
...
def earnings_reported_flag(row: dict[str, object] | pd.Series) -> bool:
    if bool(row.get("Reported Source Flag", False)):
        return True
...
def normalize_earnings_session(report_time_raw: object = None, earnings_datetime: datetime | None = None) -> str:
    try:
        ...
        missing_raw_time = report_time_raw is None
    text = "" if missing_raw_time else clean_text(str(report_time_raw)).strip()
    normalized = re.sub(r"[\s_\-]+", " ", text).strip().casefold()
    if normalized in {"", "nan", "none", "null", "unknown", "time not supplied", "time not supplied", "not supplied", "tbd", "tns", "unconfirmed", "estimated"}:
        return "Unconfirmed"
...
def earnings_surprise_payload(
    ...
    *,
    eps: bool = False,
    supplied_pct: object = None,
) -> dict[str, object]:
    actual_value = coerce_float(actual)
    estimate_value = coerce_float(estimate)
    delta = None
    pct_value = coerce_float(supplied_pct)
    tolerance = EARNINGS_EPS_TOLERANCE if eps else 1.0
    if actual_value is not None and estimate_value is not None:
        delta = actual_value - estimate_value
        if estimate_value not in (None, 0):
            denominator = abs(estimate_value) if eps else estimate_value
            if denominator not in (None, 0):
                pct_value = delta / denominator * 100
    if delta is None:
        tone = earnings_result_tone(pct=pct_value)
    ...
    if tone == "good":
        ...
        result = "In Line" if delta is not None or pct_value is not None else "N/A"
    return {"delta": delta, "pct": pct_value, "tone": tone, "result": result}


def format_earnings_surprise_delta(delta: object, *, eps: bool = False) -> str:
    return format_signed_eps(delta) if eps else format_signed_compact_currency(delta, 1)
...
def earnings_badge(delta: float | None, pct: float | None, *, unit: str) -> str:
    ...
    amount = format_signed_eps(delta) if unit == "eps" else format_signed_compact_currency(delta, 2)
    pct_text = f" ({format_percent(pct, 1, signed=True)})" if pct is not None else ""
...
def latest_quarter_period_for_event(statement_frame: pd.DataFrame, event_date: date | None) -> pd.Timestamp | None:
    if not isinstance(statement_frame, pd.DataFrame) or statement_frame.empty:
        return None
    periods = [pd.Timestamp(period) for period in statement_period_options([statement_frame])]
    if not periods:
        return None
    ...
    if (cutoff - latest).days > 180:
        return None
    return latest


def statement_value_at_period(frame: pd.DataFrame, labels: Iterable[str], period: pd.Timestamp | None) -> float | None:
    if period is None or not isinstance(frame, pd.DataFrame) or frame.empty:
        return None
    ...
    try:
        ...
        clean = clean[~pd.isna(clean.index)].sort_index()
    except Exception:
        pass
    exact = clean[clean.index == pd.Timestamp(period)]
    if not exact.empty:
        return coerce_float(exact.iloc[-1])
    same_quarter = clean[(clean.index.year == period.year) & (clean.index.quarter == period.quarter)]
    if not same_quarter.empty:
        return coerce_float(same_quarter.iloc[-1])
    before = clean[clean.index <= pd.Timestamp(period)]
    if not before.empty:
        return coerce_float(before.iloc[-1])
    return None


def prior_year_statement_value(frame: pd.DataFrame, labels: Iterable[str], period: pd.Timestamp | None) -> float | None:
    if period is None or not isinstance(frame, pd.DataFrame) or frame.empty:
        return None
    series = statement_series(frame, labels).dropna()
    if series.empty:
        return None
    try:
        series.index = pd.to_datetime(series.index, errors="coerce")
    ...
    same_quarter = series[(series.index.year == prior.year) & (series.index.quarter == prior.quarter)]
    if not same_quarter.empty:
        return coerce_float(same_quarter.iloc[-1])
    periods = list(series.index)
    try:
        position = periods.index(pd.Timestamp(period))
    except ValueError:
        position = len([candidate for candidate in periods if candidate <= pd.Timestamp(period)]) - 1
    if position >= 4:
        return coerce_float(series.iloc[position - 4])
    return None


def latest_earnings_freshness(last_updated: object) -> tuple[str, float | None]:
    updated = last_updated if isinstance(last_updated, datetime) else parse_earnings_datetime(last_updated)
    if updated is None:
        return "Unavailable", None
    if updated.tzinfo is None:
        updated = updated.replace(tzinfo=EASTERN)
    age_hours = max((eastern_now() - updated.astimezone(EASTERN)).total_seconds() / 3600, 0)
    return ("Fresh" if age_hours <= 24 else "Stale"), age_hours


@st.cache_data(ttl=86_400, show_spinner=False)
def load_latest_reported_quarterly_earnings(ticker: str) -> dict[str, object]:
    symbol = normalize_symbol(ticker)
    last_updated = eastern_now()
    if not symbol:
        return {"available": False, "ticker": "", "is_reported": False, "last_updated": last_updated, "warnings": ["Invalid ticker symbol"]}
    if yf is None:
        return {
            "available": False,
            "ticker": symbol,
            "is_reported": False,
            "source_actuals": "Yahoo Finance/yfinance unavailable",
            "source_estimates": "",
            "last_updated": last_updated,
            "warnings": [yfinance_unavailable_message()],
        }
    ...
    try:
        ...
        earnings_rows = earnings_date_rows(earnings_dates_frame(yf_ticker))
        today = eastern_now().date()
        reported_rows = [
            row
            for row in earnings_rows
            if row.get("date") and row.get("date") <= today and coerce_float(row.get("eps_actual")) is not None
        ]
        reported_rows.sort(key=lambda row: row.get("datetime") or datetime.min.replace(tzinfo=EASTERN), reverse=True)
        latest_event = reported_rows[0] if reported_rows else None

        quarterly_income = get_statement(yf_ticker, ("quarterly_income_stmt", "quarterly_financials"))
        event_date = latest_event.get("date") if latest_event else latest_past_earnings_date({"earnings_dates": earnings_dates_frame(yf_ticker)})
        period = latest_quarter_period_for_event(quarterly_income, event_date)
        revenue_actual = statement_value_at_period(quarterly_income, ("Total Revenue", "Revenue", "Operating Revenue"), period)
        statement_eps = statement_value_at_period(quarterly_income, ("Diluted EPS", "Basic EPS", "EPS Diluted", "EPS"), period)
        eps_actual = coerce_float(latest_event.get("eps_actual")) if latest_event else None
        if eps_actual is None:
            eps_actual = statement_eps
            if statement_eps is not None:
                warnings.append("EPS actual is from quarterly income statement fallback.")
        eps_estimate = coerce_float(latest_event.get("eps_estimate")) if latest_event else None
        eps_surprise_pct_source = coerce_float(latest_event.get("eps_surprise_pct")) if latest_event else None

        revenue_estimate = None
        revenue_estimate_frame = get_estimate_frame(yf_ticker, "get_revenue_estimate", "revenue_estimate")
        if latest_event and revenue_estimate_frame is not None and not revenue_estimate_frame.empty:
            # yfinance revenue estimates are often forward-looking by relative period.
            # Do not force them into a reported quarter unless they are explicitly tied to that quarter.
            warnings.append("Quarter-matched revenue estimate unavailable from current provider.")

        prior_year_revenue = prior_year_statement_value(quarterly_income, ("Total Revenue", "Revenue", "Operating Revenue"), period)
        prior_year_eps = prior_year_statement_value(quarterly_income, ("Diluted EPS", "Basic EPS", "EPS Diluted", "EPS"), period)
        if latest_event and period is not None and event_date is not None and (pd.Timestamp(event_date) - period).days > 180:
            warnings.append("Statement period is too far from earnings date; revenue actual suppressed.")
            revenue_actual = None
        revenue_payload = earnings_surprise_payload(revenue_actual, revenue_estimate)
        eps_payload = earnings_surprise_payload(eps_actual, eps_estimate, eps=True, supplied_pct=eps_surprise_pct_source)
        revenue_yoy = revenue_actual - prior_year_revenue if revenue_actual is not None and prior_year_revenue is not None else None
        revenue_yoy_pct = safe_ratio(revenue_yoy, prior_year_revenue, 100) if revenue_yoy is not None and prior_year_revenue not in (None, 0) else None
        eps_yoy = eps_actual - prior_year_eps if eps_actual is not None and prior_year_eps is not None else None
        eps_yoy_pct = safe_ratio(eps_yoy, abs(prior_year_eps), 100) if eps_yoy is not None and prior_year_eps not in (None, 0) else None
        event_dt = latest_event.get("datetime") if latest_event else None
        report_time_raw = latest_event.get("session") if latest_event else None
        normalized_session = normalize_earnings_session(report_time_raw, event_dt)
        is_reported = bool(eps_actual is not None or revenue_actual is not None)
        if latest_event is None and is_reported:
            warnings.append("Latest reported earnings date was unavailable; using latest quarterly statement period.")
        fiscal_quarter = f"Q{period.quarter}" if isinstance(period, pd.Timestamp) else None
        fiscal_year = int(period.year) if isinstance(period, pd.Timestamp) else None
        source_actuals = "Yahoo Finance/yfinance earnings dates + quarterly financials" if latest_event else "Yahoo Finance/yfinance quarterly financials"
        source_estimates = "Yahoo Finance/yfinance earnings dates" if eps_estimate is not None else "Estimate unavailable"
        available = bool(is_reported and (event_date is not None or period is not None))
        result = {
            "available": available,
            "ticker": symbol,
            "company_name": info.get("shortName") or info.get("longName") or symbol,
            "logo_url": company_logo_url(info),
            "fiscal_quarter": fiscal_quarter,
            "fiscal_year": fiscal_year,
            "period": f"{fiscal_year} {fiscal_quarter}" if fiscal_year and fiscal_quarter else "Latest reported quarter",
            "period_date": period,
            ...
            "consensus_revenue": revenue_estimate,
            "revenue_surprise": revenue_payload["delta"],
            "revenue_surprise_pct": revenue_payload["pct"],
            "eps_actual": eps_actual,
            "eps_normalized": eps_actual,
            "eps_gaap": statement_eps,
            "eps_estimate": eps_estimate,
            "consensus_eps": eps_estimate,
            "eps_surprise": eps_payload["delta"],
            "eps_surprise_pct": eps_payload["pct"],
            "eps_type": "Reported/normalized" if latest_event else "Quarterly statement EPS",
            "prior_year_revenue": prior_year_revenue,
            "prior_year_eps": prior_year_eps,
            "revenue_yoy_change": revenue_yoy,
            "revenue_yoy": revenue_yoy,
            "revenue_yoy_change_pct": revenue_yoy_pct,
            "revenue_yoy_pct": revenue_yoy_pct,
            "eps_yoy_change": eps_yoy,
            "eps_yoy": eps_yoy,
            "eps_yoy_change_pct": eps_yoy_pct,
            "eps_yoy_pct": eps_yoy_pct,
            "source_actuals": source_actuals,
            "source_estimates": source_estimates,
            "source_earnings_date": "Yahoo Finance/yfinance earnings dates" if latest_event else "Yahoo Finance/yfinance quarterly financials",
            "source_financials": "Yahoo Finance/yfinance quarterly financials",
            "source_priority_used": source_actuals,
            "last_updated": last_updated,
            "warnings": warnings,
        }
        validation_frame = normalize_earnings_data(
            [
                {
                    "Ticker": symbol,
                    ...
                    "Report Time Raw": report_time_raw,
                    "Session": normalized_session,
                    "Fiscal Quarter": fiscal_quarter,
                    "Fiscal Year": fiscal_year,
                    "Revenue Estimate": revenue_estimate,
                    "Actual Revenue": revenue_actual,
                    "EPS Estimate": eps_estimate,
                    "Actual EPS": eps_actual,
                    "Source Earnings": result["source_earnings_date"],
                    "Source Earnings Date": result["source_earnings_date"],
                    "Source Actuals": source_actuals,
                    "Source Estimates": source_estimates,
                    "Source Financials": result["source_financials"],
                    "Source Priority Used": result["source_priority_used"],
                    "Last Updated": last_updated,
                }
            ]
        )
        if not validation_frame.empty:
            validation_status = str(validation_frame.iloc[0].get("validation_status") or "Partial")
            validation_warnings = [
                warning
                for warning in str(validation_frame.iloc[0].get("validation_warnings") or "").split("; ")
                if warning
            ]
            result["validation_status"] = validation_status
            result["validation_warnings"] = validation_warnings
            result["warnings"] = list(dict.fromkeys([*warnings, *validation_warnings]))
        else:
            result["validation_status"] = "Partial"
            result["validation_warnings"] = warnings
        result["all_available_earnings_dates"] = [str(row.get("date")) for row in earnings_rows if row.get("date")]
        result["all_available_fiscal_quarters"] = [f"Q{pd.Timestamp(column).quarter} {pd.Timestamp(column).year}" for column in list(quarterly_income.columns)[:12]] if isinstance(quarterly_income, pd.DataFrame) and not quarterly_income.empty else []
        result["excluded_future_scheduled_quarters"] = len([row for row in earnings_rows if row.get("date") and row.get("date") > today and coerce_float(row.get("eps_actual")) is None])
        result["rows_excluded_due_to_annual_or_ttm_data"] = 0
        result["excluded_stale_quarters"] = max(len(reported_rows) - 1, 0)
        result["quarter_selection_reason"] = "Most recent reported earnings row with actual EPS/revenue; quarterly financials used only for same-period actual revenue fallback."
        return result
    except Exception as exc:
        return {
            "available": False,
            "ticker": symbol,
            "is_reported": False,
            "source_actuals": "Yahoo Finance/yfinance",
            "source_estimates": "",
            "last_updated": last_updated,
            "warnings": [str(exc)],
        }
...
@st.cache_data(ttl=86_400, show_spinner=False)
def load_latest_quarterly_earnings(ticker: str) -> dict[str, object]:
    """Backward-compatible alias for the shared latest reported quarter loader."""
    return load_latest_reported_quarterly_earnings(ticker)


@st.cache_data(ttl=86_400, show_spinner=False)
def refresh_all_latest_earnings(tickers: tuple[str, ...]) -> tuple[pd.DataFrame, list[dict[str, object]], datetime]:
    ...
    for ticker in tickers:
        ...
        seen.add(symbol)
        try:
            row = load_latest_reported_quarterly_earnings(symbol)
            rows.append(row)
            statuses.append({"source": symbol, "status": "OK" if row.get("available") else "Unavailable", "message": "; ".join(row.get("warnings", [])), "rows": 1 if row.get("available") else 0})
        except Exception as exc:
            statuses.append({"source": symbol, "status": "Error", "message": str(exc), "rows": 0})
    return pd.DataFrame(rows), statuses, eastern_now()


def latest_quarter_earnings_summary(ticker: str, income: pd.DataFrame, payload: dict, snapshot: dict) -> dict[str, object]:
    latest_reported = load_latest_reported_quarterly_earnings(ticker)
    if latest_reported.get("available"):
        return latest_reported
    if income.empty:
        return latest_reported if latest_reported else {"available": False}
    ...
    eps_surprise_data = earnings_surprise_payload(eps_actual, consensus_eps, eps=True)
    eps_surprise = eps_surprise_data["delta"]
    eps_surprise_pct = eps_surprise_data["pct"]
    revenue_surprise_data = earnings_surprise_payload(revenue, consensus_revenue)
    revenue_surprise = revenue_surprise_data["delta"]
    revenue_surprise_pct = revenue_surprise_data["pct"]
    ...
    return {
        ...
        "eps_yoy": eps_yoy,
        "eps_yoy_pct": eps_yoy_pct,
        "source_actuals": "Yahoo Finance/yfinance quarterly financials fallback",
        "source_estimates": "Estimate unavailable" if consensus_eps is None and consensus_revenue is None else "Yahoo Finance/yfinance analyst expectations",
        "source_earnings_date": "Derived from latest past earnings date / quarterly period",
        "source_financials": "Yahoo Finance/yfinance quarterly financials",
        "source_priority_used": "Quarterly financial statement fallback",
        "last_updated": eastern_now(),
        "is_reported": revenue is not None or eps_actual is not None,
        "validation_status": "Partial",
        "validation_warnings": ["Latest reported earnings source unavailable; using quarterly financial statement fallback."],
        "warnings": ["Latest reported earnings source unavailable; using quarterly financial statement fallback."],
        "quarter_selection_reason": "Fallback selected latest available quarterly financial statement period.",
    }


def render_latest_quarter_earnings_card(summary: dict[str, object]) -> None:
    if not summary.get("available"):
        attempted = summary.get("source_actuals") or "Yahoo Finance/yfinance"
        updated = summary.get("last_updated")
        updated_text = updated.strftime("%m/%d/%Y %I:%M %p ET").lstrip("0") if isinstance(updated, datetime) else "N/A"
        warnings = "; ".join(str(item) for item in summary.get("warnings", []) if item) or "No reported quarterly actuals returned."
        st.info(
            "Latest quarterly earnings data unavailable for this ticker. "
            f"Source attempted: {attempted}. Last refresh attempt: {updated_text}. {warnings}"
        )
        return
    ticker = str(summary.get("ticker") or "").upper()
    logo_url = str(summary.get("logo_url") or "")
    logo_html = company_logo_markup(ticker, logo_url)
    announce = summary.get("earnings_date") or summary.get("announce_date")
    announce_text = f"{announce.month}/{announce.day}/{announce.year}" if isinstance(announce, date) else "N/A"
    last_updated = summary.get("last_updated")
    freshness_status, _ = latest_earnings_freshness(last_updated)
    updated_text = last_updated.strftime("%m/%d/%Y %I:%M %p ET").lstrip("0") if isinstance(last_updated, datetime) else "N/A"
    session = summary.get("normalized_session") or "Unconfirmed"
    fiscal_period = " ".join(str(part) for part in (summary.get("fiscal_year"), summary.get("fiscal_quarter")) if part) or summary.get("period") or "Latest reported quarter"
    cards = [
        {
            "label": "Announce Date",
            "value": announce_text,
            "rows": [f"<span class='earnings-badge neutral'>{html.escape(str(session))}</span>"],
        },
        {
            "label": "Fiscal Quarter",
            "value": str(fiscal_period),
            "rows": [f"<span class='earnings-badge neutral'>{html.escape(str(summary.get('eps_type') or 'Reported/normalized'))}</span>"],
        },
        {
            "label": "Revenue Actual",
            "value": format_compact_currency(summary.get("revenue_actual", summary.get("revenue")), 2),
            "rows": [
                f"<span class='earnings-badge neutral'>Estimate {html.escape(format_compact_currency(summary.get('revenue_estimate', summary.get('consensus_revenue')), 2))}</span>",
                earnings_badge(summary.get("revenue_surprise"), summary.get("revenue_surprise_pct"), unit="revenue"),
            ],
        },
        {
            "label": "EPS Actual",
            "value": format_eps(summary.get("eps_actual", summary.get("eps_normalized"))),
            "rows": [
                f"<span class='earnings-badge neutral'>Estimate {html.escape(format_eps(summary.get('eps_estimate', summary.get('consensus_eps'))))}</span>",
                earnings_badge(summary.get("eps_surprise"), summary.get("eps_surprise_pct"), unit="eps"),
            ],
        },
        {
            "label": "Revenue YoY",
            "value": format_percent(summary.get("revenue_yoy_change_pct", summary.get("revenue_yoy_pct")), 1, signed=True),
            "rows": [
                yoy_badge(summary.get("revenue_yoy_change", summary.get("revenue_yoy")), summary.get("revenue_yoy_change_pct", summary.get("revenue_yoy_pct")), unit="revenue"),
            ],
        },
        {
            "label": "EPS YoY",
            "value": format_percent(summary.get("eps_yoy_change_pct", summary.get("eps_yoy_pct")), 1, signed=True),
            "rows": [
                yoy_badge(summary.get("eps_yoy_change", summary.get("eps_yoy")), summary.get("eps_yoy_change_pct", summary.get("eps_yoy_pct")), unit="eps"),
            ],
        },
    ]
    card_html = []
    ...
    st.markdown(
        ...
        f"<div class='latest-earnings-subtitle'>{html.escape(str(fiscal_period))} | {html.escape(freshness_status)} | Updated {html.escape(updated_text)}</div>"
        "</div>"
        ...
        f"<div class='latest-earnings-subtitle'>Source: {html.escape(str(summary.get('source_actuals') or 'N/A'))} | Estimates: {html.escape(str(summary.get('source_estimates') or 'N/A'))}</div>"
        "</div>",
        unsafe_allow_html=True,
    )
...
def render_three_statement_analysis_dashboard() -> None:
    ...
    earnings_refresh_clicked = st.sidebar.button("Refresh Earnings Data", use_container_width=True, key="three_statement_earnings_refresh")
    ...
    if refresh_clicked or earnings_refresh_clicked:
        fetch_company_financials.clear()
        fetch_home_stock_snapshot.clear()
        fetch_quote_snapshot.clear()
        fetch_performance_history.clear()
        load_latest_reported_quarterly_earnings.clear()
        load_latest_quarterly_earnings.clear()
        refresh_all_latest_earnings.clear()
    ...
    latest_earnings_cache, latest_earnings_statuses, latest_earnings_refreshed = refresh_all_latest_earnings((ticker,))
    latest_quarter_debug = (
        latest_earnings_cache.iloc[0].dropna().to_dict()
        if isinstance(latest_earnings_cache, pd.DataFrame) and not latest_earnings_cache.empty
        else latest_quarter_earnings_summary(ticker, quarterly_income_frame, payload, stock_snapshot)
    )
    if not latest_quarter_debug.get("available"):
    ...
    if period_availability_note:
        st.caption(period_availability_note)
    ...
    with st.expander("Data validation", expanded=False):
        validation_rows = [
            ...
            {"Metric": "latest_quarter_eps_normalized", "Value": format_eps(latest_quarter_debug.get("eps_normalized"))},
            {"Metric": "latest_quarter_eps_gaap", "Value": format_eps(latest_quarter_debug.get("eps_gaap"))},
            {"Metric": "latest_quarter_consensus_revenue", "Value": format_compact_currency(latest_quarter_debug.get("consensus_revenue"), 2)},
            {"Metric": "latest_quarter_consensus_eps", "Value": format_eps(latest_quarter_debug.get("consensus_eps"))},
            ...
            {"Metric": "latest_quarter_prior_year_eps", "Value": format_eps(latest_quarter_debug.get("prior_year_eps"))},
    ...
    with st.expander("Latest Earnings Data Validation", expanded=False):
        latest_status, cache_age_hours = latest_earnings_freshness(latest_quarter_debug.get("last_updated"))
        latest_validation_rows = [
            {"Metric": "selected_ticker", "Value": ticker},
            {"Metric": "all_available_earnings_dates", "Value": ", ".join(str(item) for item in latest_quarter_debug.get("all_available_earnings_dates", [])[:12]) or "N/A"},
            {"Metric": "all_available_fiscal_quarters", "Value": ", ".join(str(item) for item in latest_quarter_debug.get("all_available_fiscal_quarters", [])[:12]) or "N/A"},
            {"Metric": "selected_latest_earnings_date", "Value": str(latest_quarter_debug.get("earnings_date") or latest_quarter_debug.get("announce_date") or "N/A")},
            {"Metric": "selected_latest_fiscal_quarter", "Value": f"{latest_quarter_debug.get('fiscal_year') or ''} {latest_quarter_debug.get('fiscal_quarter') or ''}".strip() or "N/A"},
            {"Metric": "selected_latest_revenue_actual", "Value": latest_quarter_debug.get("revenue_actual", latest_quarter_debug.get("revenue"))},
            {"Metric": "selected_latest_eps_actual", "Value": latest_quarter_debug.get("eps_actual", latest_quarter_debug.get("eps_normalized"))},
            {"Metric": "selected_revenue_estimate", "Value": latest_quarter_debug.get("revenue_estimate", latest_quarter_debug.get("consensus_revenue"))},
            {"Metric": "selected_eps_estimate", "Value": latest_quarter_debug.get("eps_estimate", latest_quarter_debug.get("consensus_eps"))},
            {"Metric": "selected_revenue_surprise", "Value": latest_quarter_debug.get("revenue_surprise")},
            {"Metric": "selected_eps_surprise", "Value": latest_quarter_debug.get("eps_surprise")},
            {"Metric": "latest_earnings_date", "Value": str(latest_quarter_debug.get("earnings_date") or latest_quarter_debug.get("announce_date") or "N/A")},
            {"Metric": "fiscal_quarter", "Value": latest_quarter_debug.get("fiscal_quarter") or "N/A"},
            {"Metric": "fiscal_year", "Value": latest_quarter_debug.get("fiscal_year") or "N/A"},
            {"Metric": "is_reported", "Value": str(bool(latest_quarter_debug.get("is_reported")))},
            {"Metric": "raw_revenue_actual", "Value": latest_quarter_debug.get("revenue_actual", latest_quarter_debug.get("revenue"))},
            {"Metric": "raw_revenue_estimate", "Value": latest_quarter_debug.get("revenue_estimate", latest_quarter_debug.get("consensus_revenue"))},
            {"Metric": "revenue_surprise", "Value": latest_quarter_debug.get("revenue_surprise")},
            {"Metric": "revenue_surprise_pct", "Value": latest_quarter_debug.get("revenue_surprise_pct")},
            {"Metric": "raw_eps_actual", "Value": latest_quarter_debug.get("eps_actual", latest_quarter_debug.get("eps_normalized"))},
            {"Metric": "raw_eps_estimate", "Value": latest_quarter_debug.get("eps_estimate", latest_quarter_debug.get("consensus_eps"))},
            {"Metric": "eps_surprise", "Value": latest_quarter_debug.get("eps_surprise")},
            {"Metric": "eps_surprise_pct", "Value": latest_quarter_debug.get("eps_surprise_pct")},
            {"Metric": "eps_type", "Value": latest_quarter_debug.get("eps_type") or "N/A"},
            {"Metric": "prior_year_revenue", "Value": latest_quarter_debug.get("prior_year_revenue")},
            {"Metric": "prior_year_eps", "Value": latest_quarter_debug.get("prior_year_eps")},
            {"Metric": "normalized_session", "Value": latest_quarter_debug.get("normalized_session") or "Unconfirmed"},
            {"Metric": "source_actuals", "Value": latest_quarter_debug.get("source_actuals") or "N/A"},
            {"Metric": "source_estimates", "Value": latest_quarter_debug.get("source_estimates") or "N/A"},
            {"Metric": "source_earnings_date", "Value": latest_quarter_debug.get("source_earnings_date") or "N/A"},
            {"Metric": "last_updated", "Value": latest_quarter_debug.get("last_updated") or "N/A"},
            {"Metric": "cache_age_hours", "Value": format_number(cache_age_hours, 2)},
            {"Metric": "freshness_status", "Value": latest_status},
            {"Metric": "validation_status", "Value": latest_quarter_debug.get("validation_status") or "Partial"},
            {"Metric": "validation_warnings", "Value": "; ".join(str(item) for item in latest_quarter_debug.get("validation_warnings", []) if item) or "None"},
            {"Metric": "excluded_stale_quarters", "Value": latest_quarter_debug.get("excluded_stale_quarters", "N/A")},
            {"Metric": "excluded_future_scheduled_quarters", "Value": latest_quarter_debug.get("excluded_future_scheduled_quarters", "N/A")},
            {"Metric": "rows_excluded_due_to_annual_or_ttm_data", "Value": latest_quarter_debug.get("rows_excluded_due_to_annual_or_ttm_data", "N/A")},
            {"Metric": "quarter_selection_reason", "Value": latest_quarter_debug.get("quarter_selection_reason", "N/A")},
            {"Metric": "warnings", "Value": "; ".join(str(item) for item in latest_quarter_debug.get("warnings", []) if item) or "None"},
            {"Metric": "cache_rows_loaded", "Value": len(latest_earnings_cache) if isinstance(latest_earnings_cache, pd.DataFrame) else 0},
            {"Metric": "cache_status", "Value": "; ".join(f"{row.get('source')}: {row.get('status')}" for row in latest_earnings_statuses[:5]) if latest_earnings_statuses else "N/A"},
        ]
        render_dashboard_table(pd.DataFrame(latest_validation_rows), height=420)
...
def earnings_session_for_datetime(value: datetime | None) -> str:
    return normalize_earnings_session(None, value)
...
def earnings_session_for_code(value: object, fallback_datetime: datetime | None = None) -> str:
    return normalize_earnings_session(value, fallback_datetime)
...
def yahoo_calendar_item_to_earnings_row(item: dict, week_start: date, week_end: date) -> dict[str, object] | None:
    ...
    report_time_raw = first_present_value(item, ("startdatetimetype", "startDateTimeType", "timeType", "session", "time"))
    session = earnings_session_for_code(report_time_raw, event_dt)
    source_reported = str(first_present_value(item, ("reported", "isReported", "status", "eventStatus")) or "").strip().casefold()
    reported_flag = source_reported in {"reported", "complete", "completed", "true", "1", "actual", "results reported"}
    return {
        "Ticker": symbol,
        "Company": company,
        "Earnings Date": event_date,
        ...
        "Reported": bool(actual_revenue is not None or eps_actual is not None or reported_flag),
        "Reported Source Flag": reported_flag,
        "EPS Type": "Reported/normalized",
        "Revenue Currency": "USD",
        "Revenue Unit": "raw",
        "Source Earnings": "Yahoo Finance earnings calendar",
        "Source Estimates": "Yahoo Finance earnings calendar",
        "Source Actuals": "Yahoo Finance earnings calendar" if actual_revenue is not None or eps_actual is not None else "",
        "Source Report Time": str(item.get("Source Report Time") or "Yahoo Finance earnings calendar"),
...
YAHOO_VISUALIZATION_EARNINGS_FIELDS = (
    "ticker",
    "companyshortname",
    "eventname",
    "startdatetime",
...
def yahoo_visualization_earnings_body(day: date, *, offset: int = 0, size: int = 250) -> dict[str, object]:
    next_day = day + timedelta(days=1)
    return {
        "sortType": "DESC",
        "entityIdType": "sp_earnings",
        "sortField": "intradaymarketcap",
        "includeFields": list(YAHOO_VISUALIZATION_EARNINGS_FIELDS),
        "query": {
            "operator": "and",
            "operands": [
                {"operator": "gte", "operands": ["startdatetime", day.isoformat()]},
                {"operator": "lt", "operands": ["startdatetime", next_day.isoformat()]},
...
def yahoo_visualization_rows(payload: object) -> tuple[list[dict[str, object]], int | None]:
    if not isinstance(payload, dict):
        return [], None
    results = nested_value(payload, ("finance", "result"))
    if not isinstance(results, list) or not results:
        return [], None
    result = results[0] if isinstance(results[0], dict) else {}
    total = coerce_float(result.get("total"))
...
def yahoo_visualization_session() -> tuple[requests.Session, str | None]:
    session = requests.Session()
    session.headers.update({"User-Agent": "Mozilla/5.0 (compatible; StreamlitInvestmentDashboard/1.0)"})
    try:
        session.get("https://fc.yahoo.com", timeout=8)
        crumb_response = session.get("https://query1.finance.yahoo.com/v1/test/getcrumb", timeout=8)
        crumb_response.raise_for_status()
        crumb = clean_text(crumb_response.text).strip()
        return session, crumb or None
    except Exception:
        return session, None
...
def fetch_yahoo_weekly_earnings_calendar(week_start_iso: str, week_end_iso: str) -> tuple[list[dict[str, object]], list[dict[str, object]], datetime]:
    ...
    rows: list[dict[str, object]] = []
    statuses: list[dict[str, object]] = []
    session, crumb = yahoo_visualization_session()
    url = "https://query1.finance.yahoo.com/v1/finance/visualization"
    if not crumb:
        statuses.append({"source": "Yahoo Finance visualization earnings calendar", "status": "Error", "message": "Unable to acquire Yahoo crumb", "rows": 0})
        ...
        for offset in range(0, 1000, 250):
            try:
                response = session.post(
                    ...
                    json=yahoo_visualization_earnings_body(day, offset=offset, size=250),
                    timeout=15,
                )
                response.raise_for_status()
                items, total = yahoo_visualization_rows(response.json())
                day_total = total if total is not None else day_total
            except Exception as exc:
                statuses.append({"source": "Yahoo Finance visualization earnings calendar", "status": "Error", "message": f"{day.isoformat()}: {exc}", "rows": 0})
                break
            parsed_rows = []
            for item in items:
                item["Source Report Time"] = "Yahoo Finance visualization calendar"
                row = yahoo_calendar_item_to_earnings_row(item, week_start, week_end)
                if row:
                    row["Market Cap"] = coerce_float(item.get("intradaymarketcap"))
                    row["Source Earnings"] = "Yahoo Finance visualization earnings calendar"
                    row["Source Estimates"] = "Yahoo Finance visualization earnings calendar"
                    row["Source Actuals"] = "Yahoo Finance visualization earnings calendar" if row.get("Actual EPS") is not None or row.get("Actual Revenue") is not None else ""
                    row["Source Report Time"] = "Yahoo Finance visualization calendar"
                    row["Data Notes"] = "Yahoo visualization earnings calendar row"
                    key = f"{row.get('Ticker')}|{row.get('Earnings Date')}"
                    if key not in seen:
                        seen.add(key)
                        parsed_rows.append(row)
            rows.extend(parsed_rows)
            statuses.append({"source": "Yahoo Finance visualization earnings calendar", "status": "OK", "message": f"{day.isoformat()} offset {offset}; total {day_total if day_total is not None else 'N/A'}", "rows": len(parsed_rows)})
            if len(items) < 250 or (day_total is not None and offset + 250 >= day_total):
                break
...
def merge_earnings_calendar_hint(row: dict[str, object], calendar_row: dict[str, object]) -> dict[str, object]:
    merged = dict(row)
    incoming_dt = parse_earnings_datetime(calendar_row.get("Earnings DateTime"))
    existing_dt = parse_earnings_datetime(merged.get("Earnings DateTime"))
    incoming_raw = calendar_row.get("Report Time Raw") if not earnings_missing_value(calendar_row.get("Report Time Raw")) else calendar_row.get("Session")
    existing_raw = merged.get("Report Time Raw") if not earnings_missing_value(merged.get("Report Time Raw")) else merged.get("Session")
    incoming_session = normalize_earnings_session(incoming_raw, incoming_dt)
    existing_session = normalize_earnings_session(existing_raw, existing_dt)
    incoming_has_intraday_time = incoming_dt is not None and not (incoming_dt.hour == 0 and incoming_dt.minute == 0)
    existing_has_intraday_time = existing_dt is not None and not (existing_dt.hour == 0 and existing_dt.minute == 0)
    if incoming_session in PRIMARY_EARNINGS_SESSIONS and (
        existing_session == "Unconfirmed" or (incoming_has_intraday_time and not existing_has_intraday_time)
    ):
        merged["Session"] = incoming_session
        if incoming_dt is not None:
            merged["Earnings DateTime"] = incoming_dt
        if not earnings_missing_value(calendar_row.get("Report Time Raw")):
            merged["Report Time Raw"] = calendar_row.get("Report Time Raw")
        if not earnings_missing_value(calendar_row.get("Source Report Time")):
            merged["Source Report Time"] = calendar_row.get("Source Report Time")
...
def nasdaq_session_label(value: object) -> str:
    return normalize_earnings_session(value)
...
def nasdaq_calendar_item_to_earnings_row(item: dict, event_date: date) -> dict[str, object] | None:
    ...
    report_time_raw = first_present_value(item, ("time", "startdatetimetype", "session"))
    source_reported = str(first_present_value(item, ("reported", "isReported", "status")) or "").strip().casefold()
    reported_flag = source_reported in {"reported", "complete", "completed", "true", "1", "actual", "results reported"}
    return {
        "Ticker": symbol,
        "Company": company,
        ...
        "Report Time Raw": report_time_raw,
        "Session": nasdaq_session_label(report_time_raw),
        "Sector": "Unknown",
        ...
        "EPS Surprise %": eps_surprise_pct,
        "Reported": bool(eps_actual is not None or reported_flag),
        "Reported Source Flag": reported_flag,
        "EPS Type": "Reported/normalized",
        "Revenue Currency": "USD",
        "Revenue Unit": "raw",
        "Source Earnings": "Nasdaq earnings calendar",
        "Source Estimates": "Nasdaq earnings calendar",
        "Source Actuals": "Nasdaq earnings calendar" if eps_actual is not None else "",
...
def earnings_event_from_calendar_hint(calendar_hint: dict[str, object] | None, week_start: date, week_end: date) -> dict[str, object] | None:
    ...
    return {
        "datetime": event_dt,
        "date": event_date,
        "session": normalize_earnings_session(calendar_hint.get("Report Time Raw") or calendar_hint.get("Session"), event_dt),
...
def latest_reported_statement_revenue(yf_ticker: yf.Ticker, event_date: date | None) -> float | None:
    ...
    if event_date is not None:
        try:
            ...
            if candidates.empty:
                return None
            candidate_date = pd.Timestamp(candidates.index[-1]).date()
            if (event_date - candidate_date).days > 120:
                return None
...
def refine_earnings_event_from_yfinance(
    yf_ticker: yf.Ticker,
    ...
    try:
        ...
        return event, False, ""
    event_rows = [row for row in earnings_date_rows(earnings_frame) if row.get("date") == event.get("date")]
    if not event_rows:
        return event, False, ""
    event_rows.sort(
        key=lambda row: (
            0 if row.get("session") in PRIMARY_EARNINGS_SESSIONS else 1,
            row.get("datetime") or datetime.max.replace(tzinfo=EASTERN),
        )
    )
    matched = event_rows[0]
    refined = dict(event)
    changed = False
    if normalize_earnings_session(refined.get("session")) == "Unconfirmed" and matched.get("session") != "Unconfirmed":
        refined["session"] = matched.get("session")
        refined["datetime"] = matched.get("datetime") or refined.get("datetime")
        changed = True
    for key in ("eps_estimate", "eps_actual", "eps_surprise_pct"):
        if coerce_float(refined.get(key)) is None and coerce_float(matched.get(key)) is not None:
            refined[key] = coerce_float(matched.get(key))
            changed = True
    return refined, changed, "Yahoo Finance/yfinance earnings timestamp" if changed else ""
...
def fetch_single_earnings_release(
    ...
    try:
        ...
        if event is None:
            ...
            event = event_rows[0]
            session_source = "Yahoo Finance/yfinance earnings dates"
        else:
            event, _, session_source = refine_earnings_event_from_yfinance(yf_ticker, event)
        info: dict = {}
        try:
            fast_info = getattr(yf_ticker, "fast_info", {}) or {}
            info.update(dict(fast_info))
        ...
        source_reported = bool((calendar_hint or {}).get("Reported Source Flag"))
        is_reported = bool(eps_actual is not None or actual_revenue is not None or source_reported)
        actual_source = str((calendar_hint or {}).get("Source Actuals") or "").strip()
        if actual_revenue is None and is_reported:
            ...
            if actual_revenue is not None:
                actual_source = "Yahoo Finance/yfinance quarterly income statement"
        eps_surprise = earnings_surprise_payload(eps_actual, eps_estimate, eps=True, supplied_pct=event.get("eps_surprise_pct"))
        revenue_surprise = earnings_surprise_payload(actual_revenue, revenue_est)
        ...
        if session_source:
            note_parts.append(f"report time confirmed by {session_source}")
        return {
            "Ticker": symbol,
            "Company": company,
            "Earnings Date": event["date"],
            "Earnings DateTime": event["datetime"],
            "Report Time Raw": (calendar_hint or {}).get("Report Time Raw") or event.get("session"),
            ...
            "Actual Revenue": actual_revenue,
            "Actual EPS": eps_actual,
            "Revenue Surprise %": revenue_surprise["pct"],
            "EPS Surprise %": eps_surprise["pct"],
            "Reported": is_reported,
            "Reported Source Flag": source_reported,
            "EPS Type": "Reported/normalized",
            "Revenue Currency": "USD",
            "Revenue Unit": "raw",
            "Source Earnings": (calendar_hint or {}).get("Source Earnings") or ("Yahoo Finance/yfinance earnings dates" if event_rows else "Earnings calendar"),
            "Source Estimates": (calendar_hint or {}).get("Source Estimates") or "Yahoo Finance/yfinance analyst estimates",
            "Source Actuals": actual_source,
            "Source Report Time": session_source or (calendar_hint or {}).get("Source Report Time") or "",
...
@st.cache_data(ttl=86_400, show_spinner=False)
def load_scheduled_earnings_calendar(week_start_iso: str, week_end_iso: str) -> tuple[pd.DataFrame, list[dict[str, object]], datetime]:
    ...
    frame = normalize_earnings_data(rows)
...
def load_weekly_earnings_calendar(week_start_iso: str, week_end_iso: str) -> tuple[pd.DataFrame, list[dict[str, object]], datetime]:
    return load_scheduled_earnings_calendar(week_start_iso, week_end_iso)


@st.cache_data(ttl=900, show_spinner=False)
def load_stocktwits_discussion(tickers: tuple[str, ...]) -> tuple[pd.DataFrame, list[dict[str, object]], datetime]:
    rows: list[dict[str, object]] = []
    statuses: list[dict[str, object]] = []
    seen: set[str] = set()
    for ticker in tickers[:150]:
        ...
        seen.add(symbol)
        try:
            mentions, status = request_stocktwits_stream(symbol)
            statuses.append(status)
        except Exception as exc:
            mentions = []
            statuses.append({"source": f"Stocktwits {symbol}", "status": "Error", "message": str(exc), "rows": 0})
        bullish = sum(1 for mention in mentions if str(mention.sentiment).casefold() == "bullish")
        bearish = sum(1 for mention in mentions if str(mention.sentiment).casefold() == "bearish")
        total_sentiment = bullish + bearish
        rows.append(
            {
                "Ticker": symbol,
                "ticker": symbol,
                "stocktwits_mentions": len(mentions),
                "stocktwits_sentiment": safe_ratio(bullish, total_sentiment, 100) if total_sentiment else None,
                "stocktwits_bullish_count": bullish,
                "stocktwits_bearish_count": bearish,
                "stocktwits_trending_rank": None,
                "stocktwits_last_updated": eastern_now(),
            }
        )
    frame = pd.DataFrame(rows)
    if not frame.empty and "stocktwits_mentions" in frame:
        frame["stocktwits_trending_rank"] = frame["stocktwits_mentions"].rank(ascending=False, method="min").astype("Int64")
    return frame, statuses, eastern_now()
...
def render_earnings_detail_table(frame: pd.DataFrame, height: int) -> None:
    ...
    for column in eps_columns:
        display[column] = pd.to_numeric(display[column], errors="coerce").map(lambda value: format_eps(value))
    display = display.astype("object").where(pd.notna(display), "N/A")
    styles = [
        {
            "selector": "th",
...
def earnings_metric_box(label: str, value: str, tone: str = "neutral") -> str:
    safe_tone = tone if tone in {"good", "bad", "neutral"} else "neutral"
    safe_value = "-" if str(value).strip() in {"â€”", "—", "&mdash;"} else str(value)
    return (
        f"<div class='earnings-metric-pill {html.escape(safe_tone)}'>"
        f"<span>{html.escape(label)}</span>"
...
def earnings_surprise_components(
    ...
    *,
    ...
    payload = earnings_surprise_payload(actual, estimate, eps=eps, supplied_pct=pct)
    return payload["delta"], payload["pct"], payload["tone"]
...
def earnings_surprise_badge(prefix: str, actual: object, estimate: object, pct: object = None, *, eps: bool = False) -> str | None:
    ...
    delta_text = format_signed_eps(delta) if eps else format_signed_compact_currency(delta, 1)
...
def validate_earnings_row(row: pd.Series | dict) -> tuple[str, list[str]]:
    """Validate one normalized earnings row without blocking partial display."""
    warnings: list[str] = []
    def row_value(*names: str) -> object:
        for name in names:
            value = row.get(name)
            if not earnings_missing_value(value):
                return value
        return None

    ticker = str(row_value("Ticker", "ticker") or "").strip().upper()
    earnings_date = parse_date(row_value("Earnings Date", "earnings_date"))
    raw_time = row_value("Report Time Raw")
    if earnings_missing_value(raw_time):
        raw_time = row_value("Session")
    if earnings_missing_value(raw_time):
        raw_time = row_value("session")
    session = normalize_earnings_session(raw_time, row.get("Earnings DateTime"))
    if not ticker:
        warnings.append("Missing ticker.")
    if earnings_date is None:
        warnings.append("Missing earnings date.")
    if session not in EARNINGS_SESSIONS:
        warnings.append(f"Unexpected session classification: {session}.")

    eps_actual = coerce_float(row_value("Actual EPS", "eps_actual"))
    eps_estimate = coerce_float(row_value("EPS Estimate", "eps_estimate"))
    revenue_actual = coerce_float(row_value("Actual Revenue", "revenue_actual"))
    revenue_estimate = coerce_float(row_value("Revenue Estimate", "revenue_estimate"))
    if eps_actual is not None and eps_actual < 0 and "-" not in format_eps(eps_actual):
        warnings.append("Negative actual EPS display lost sign.")
    if eps_estimate is not None and eps_estimate < 0 and "-" not in format_eps(eps_estimate):
        warnings.append("Negative EPS estimate display lost sign.")

    eps_payload = earnings_surprise_payload(eps_actual, eps_estimate, eps=True)
    revenue_payload = earnings_surprise_payload(revenue_actual, revenue_estimate)
    if eps_payload["delta"] is not None:
        if eps_payload["delta"] < -EARNINGS_EPS_TOLERANCE and eps_payload["result"] != "Miss":
            warnings.append("EPS surprise sign conflicts with beat/miss label.")
        if eps_payload["delta"] > EARNINGS_EPS_TOLERANCE and eps_payload["result"] != "Beat":
            warnings.append("EPS surprise sign conflicts with beat/miss label.")
    if revenue_payload["delta"] is not None:
        if revenue_payload["delta"] < -1 and revenue_payload["result"] != "Miss":
            warnings.append("Revenue surprise sign conflicts with beat/miss label.")
        if revenue_payload["delta"] > 1 and revenue_payload["result"] != "Beat":
            warnings.append("Revenue surprise sign conflicts with beat/miss label.")

    fiscal_quarter = row_value("Fiscal Quarter", "fiscal_quarter")
    fiscal_year = row_value("Fiscal Year", "fiscal_year")
    if (eps_actual is not None or revenue_actual is not None) and (earnings_missing_value(fiscal_quarter) or earnings_missing_value(fiscal_year)):
        warnings.append("Fiscal quarter/year unavailable; provider did not return quarter-matching metadata.")
    options_expiry = parse_date(row_value("Options Expiry Used", "options_expiry_used"))
    if options_expiry is not None and earnings_date is not None and options_expiry == earnings_date:
        warnings.append("Options expiry equals earnings date; UI must label them separately.")

    source_actuals = row_value("Source Actuals", "source_actuals")
    source_estimates = row_value("Source Estimates", "source_estimates")
    if (eps_actual is not None or revenue_actual is not None) and earnings_missing_value(source_actuals):
        warnings.append("Actuals source unavailable.")
    if (eps_estimate is not None or revenue_estimate is not None) and earnings_missing_value(source_estimates):
        warnings.append("Estimates source unavailable.")

    if not ticker or earnings_date is None:
...
def normalize_earnings_data(
    raw_earnings_data: pd.DataFrame | list[dict[str, object]] | tuple[dict[str, object], ...] | None,
    raw_estimates_data: pd.DataFrame | list[dict[str, object]] | tuple[dict[str, object], ...] | None = None,
    raw_actuals_data: pd.DataFrame | list[dict[str, object]] | tuple[dict[str, object], ...] | None = None,
) -> pd.DataFrame:
    """Shared earnings normalizer used by the calendar and Company Analysis."""
    frames: list[pd.DataFrame] = []
    for raw in (raw_earnings_data, raw_estimates_data, raw_actuals_data):
        if raw is None:
            continue
    ...
    if "Ticker" in merged and "Earnings Date" in merged:
        merged["__earnings_date_key"] = merged["Earnings Date"].map(parse_date)
        merged = merged.sort_values(["Ticker", "__earnings_date_key"], na_position="last")
        merged = merged.groupby(["Ticker", "__earnings_date_key"], dropna=False, as_index=False).first()
        merged = merged.drop(columns=["__earnings_date_key"], errors="ignore")
    return normalize_earnings_dataframe(merged)


def normalize_earnings_dataframe(frame: pd.DataFrame) -> pd.DataFrame:
    if not isinstance(frame, pd.DataFrame) or frame.empty:
    ...
    sessions = []
    reported_flags = []
    eps_surprise_dollars = []
    eps_surprise_pcts = []
    revenue_surprise_dollars = []
    revenue_surprise_pcts = []
    validation_warnings: list[str] = []
    for index, row in normalized.iterrows():
        event_dt = parse_earnings_datetime(row.get("Earnings DateTime"))
        raw_time = row.get("Report Time Raw")
        if earnings_missing_value(raw_time):
            raw_time = row.get("Session")
        session = normalize_earnings_session(raw_time, event_dt)
        sessions.append(session)
        eps_payload = earnings_surprise_payload(row.get("Actual EPS"), row.get("EPS Estimate"), eps=True)
        revenue_payload = earnings_surprise_payload(row.get("Actual Revenue"), row.get("Revenue Estimate"))
        eps_surprise_dollars.append(eps_payload["delta"])
        eps_surprise_pcts.append(eps_payload["pct"])
        revenue_surprise_dollars.append(revenue_payload["delta"])
        revenue_surprise_pcts.append(revenue_payload["pct"])
        reported = earnings_reported_flag(row)
        reported_flags.append(reported)
        ticker = row.get("Ticker") or f"row {index}"
        if coerce_float(row.get("Actual EPS")) is not None and coerce_float(row.get("Actual EPS")) < 0 and "-" not in format_eps(row.get("Actual EPS")):
            validation_warnings.append(f"{ticker}: negative actual EPS display lost sign")
        if coerce_float(row.get("EPS Estimate")) is not None and coerce_float(row.get("EPS Estimate")) < 0 and "-" not in format_eps(row.get("EPS Estimate")):
            validation_warnings.append(f"{ticker}: negative EPS estimate display lost sign")
        if eps_payload["delta"] is not None and eps_payload["delta"] < -EARNINGS_EPS_TOLERANCE and eps_payload["tone"] != "bad":
            validation_warnings.append(f"{ticker}: EPS miss classification mismatch")
        if eps_payload["delta"] is not None and eps_payload["delta"] > EARNINGS_EPS_TOLERANCE and eps_payload["tone"] != "good":
            validation_warnings.append(f"{ticker}: EPS beat classification mismatch")
        if revenue_payload["delta"] is not None and revenue_payload["delta"] < -1 and revenue_payload["tone"] != "bad":
            validation_warnings.append(f"{ticker}: revenue miss classification mismatch")
        if revenue_payload["delta"] is not None and revenue_payload["delta"] > 1 and revenue_payload["tone"] != "good":
            validation_warnings.append(f"{ticker}: revenue beat classification mismatch")
    normalized["Session"] = sessions
    normalized["Reported"] = reported_flags
    normalized["EPS Surprise $"] = eps_surprise_dollars
    normalized["EPS Surprise %"] = eps_surprise_pcts
    normalized["Revenue Surprise $"] = revenue_surprise_dollars
    normalized["Revenue Surprise %"] = revenue_surprise_pcts
    normalized["ticker"] = normalized["Ticker"].astype(str)
    normalized["company_name"] = normalized["Company"]
    normalized["earnings_date"] = normalized["Earnings Date"]
    normalized["fiscal_quarter"] = normalized.get("Fiscal Quarter", pd.Series(index=normalized.index, dtype=object))
    normalized["fiscal_year"] = normalized.get("Fiscal Year", pd.Series(index=normalized.index, dtype=object))
    normalized["report_time_raw"] = normalized["Report Time Raw"]
    normalized["session"] = normalized["Session"]
    normalized["is_reported"] = normalized["Reported"]
    normalized["eps_estimate"] = pd.to_numeric(normalized["EPS Estimate"], errors="coerce")
    normalized["eps_actual"] = pd.to_numeric(normalized["Actual EPS"], errors="coerce")
    normalized["eps_type"] = normalized["EPS Type"].fillna("Reported/normalized")
    normalized["revenue_estimate"] = pd.to_numeric(normalized["Revenue Estimate"], errors="coerce")
    normalized["revenue_actual"] = pd.to_numeric(normalized["Actual Revenue"], errors="coerce")
    normalized["revenue_currency"] = normalized["Revenue Currency"].fillna("USD")
    normalized["revenue_unit"] = normalized["Revenue Unit"].fillna("raw")
    normalized["eps_surprise"] = pd.to_numeric(normalized["EPS Surprise $"], errors="coerce")
    normalized["eps_surprise_pct"] = pd.to_numeric(normalized["EPS Surprise %"], errors="coerce")
    normalized["revenue_surprise"] = pd.to_numeric(normalized["Revenue Surprise $"], errors="coerce")
    normalized["revenue_surprise_pct"] = pd.to_numeric(normalized["Revenue Surprise %"], errors="coerce")
    normalized["source_earnings"] = normalized["Source Earnings"]
    normalized["source_earnings_date"] = normalized["Source Earnings Date"].astype("object").where(
        pd.notna(normalized["Source Earnings Date"]),
        normalized["Source Earnings"],
    )
    normalized["source_estimates"] = normalized["Source Estimates"]
    normalized["source_actuals"] = normalized["Source Actuals"]
    normalized["source_financials"] = normalized["Source Financials"]
    normalized["source_priority_used"] = normalized["Source Priority Used"].astype("object").where(
        pd.notna(normalized["Source Priority Used"]),
        normalized["Source Earnings"],
    )
    normalized["source_report_time"] = normalized["Source Report Time"]
    normalized["last_updated"] = normalized["Last Updated"].astype("object").where(pd.notna(normalized["Last Updated"]), eastern_now())
    row_statuses: list[str] = []
    row_warning_text: list[str] = []
    for _, row in normalized.iterrows():
        status, warnings_for_row = validate_earnings_row(row)
        row_statuses.append(status)
        row_warning_text.append("; ".join(warnings_for_row))
        validation_warnings.extend(f"{row.get('Ticker') or 'row'}: {warning}" for warning in warnings_for_row)
    normalized["validation_status"] = row_statuses
    normalized["validation_warnings"] = row_warning_text
    normalized.attrs["validation_warnings"] = validation_warnings
    return normalized


def earnings_calculation_test_cases() -> pd.DataFrame:
    rows = []
    for estimate, actual, expected in (
        (-0.02, -0.04, "EPS Miss -$0.02 / -100.0%"),
        (-0.04, -0.02, "EPS Beat +$0.02 / +50.0%"),
        (0.02, 0.08, "EPS Beat +$0.06 / +300.0%"),
        (0.08, 0.02, "EPS Miss -$0.06 / -75.0%"),
    ):
        payload = earnings_surprise_payload(actual, estimate, eps=True)
        rows.append(
            {
                "Case": expected,
                "Actual": format_eps(actual),
                "Estimate": format_eps(estimate),
                "Observed": f"EPS {payload['result']} {format_signed_eps(payload['delta'])} / {format_percent(payload['pct'], 1, signed=True)}",
            }
        )
    for estimate, actual, expected in (
        (28_300_000, 24_100_000, "Rev Miss -$4.2M / -14.8%"),
        (25_700_000, 28_500_000, "Rev Beat +$2.8M / +10.9%"),
    ):
        payload = earnings_surprise_payload(actual, estimate)
        rows.append(
            {
                "Case": expected,
                "Actual": format_compact_currency(actual, 1),
                "Estimate": format_compact_currency(estimate, 1),
                "Observed": f"Rev {payload['result']} {format_signed_compact_currency(payload['delta'], 1)} / {format_percent(payload['pct'], 1, signed=True)}",
            }
        )
    for raw, expected in (
        ("BMO", "Before Open"),
...
def render_earnings_card(row: pd.Series) -> str:
    ...
    expectation_html = "".join(
        [
            earnings_metric_box("7D Move", format_move(implied_move, 1) if implied_move is not None else "N/A", "good" if implied_move is not None else "neutral"),
            earnings_metric_box("Rev Est", format_compact_currency(revenue_estimate, 1) if revenue_estimate is not None else "N/A", "neutral"),
    ...
    actual_html = "".join(
        [
            earnings_metric_box(
                ...
                revenue_tone if revenue_estimate is not None and actual_revenue is not None else "neutral",
            ),
            earnings_metric_box(
                "Actual EPS",
                format_eps(actual_eps) if actual_eps is not None else "-",
                eps_tone if eps_estimate is not None and actual_eps is not None else "neutral",
            ),
        ]
    )
    surprise_badges = []
    ...
    stocktwits_mentions = coerce_float(row.get("stocktwits_mentions"))
    if stocktwits_mentions is not None and stocktwits_mentions > 0:
        surprise_badges.append(earnings_mini_badge(f"ST {compact_number(stocktwits_mentions)} mentions", "neutral"))
    stocktwits_sentiment = coerce_float(row.get("stocktwits_sentiment"))
    if stocktwits_sentiment is not None:
        surprise_badges.append(earnings_mini_badge(f"ST Bullish {format_percent(stocktwits_sentiment, 0)}", "neutral"))
    stocktwits_rank = coerce_float(row.get("stocktwits_trending_rank"))
    if stocktwits_rank is not None and stocktwits_rank > 0:
        surprise_badges.append(earnings_mini_badge(f"ST Rank #{format_number(stocktwits_rank, 0)}", "neutral"))
    earnings_date_badge = (
...
def render_earnings_summary(frame: pd.DataFrame, week_start: date, total_frame: pd.DataFrame | None = None) -> None:
    ...
    if "Reported" in frame:
        reported_mask = frame["Reported"].fillna(False).astype(bool)
    upcoming_mask = (
...
def earnings_bucket_label(day: date, session: str, count: int) -> str:
    return f"{calendar.day_name[day.weekday()]} {day.strftime('%b %d')} | {session} ({count:,})"


def earnings_bucket_options(frame: pd.DataFrame, week_start: date, session_filter: str, show_time_not_supplied: bool) -> list[str]:
    ...
    for day in days:
        ...
        if day_rows.empty:
            continue
        if session_filter in PRIMARY_EARNINGS_SESSIONS:
        ...
        for session in sessions:
            count = int(day_rows["Session"].eq(session).sum())
            if count:
                labels.append(earnings_bucket_label(day, session, count))
    return labels
...
def earnings_unconfirmed_rows_for_enrichment(frame: pd.DataFrame, limit: int) -> pd.DataFrame:
    ...
    if limit <= 0:
        return frame.head(0)
    unconfirmed = frame[~frame["Session"].isin(PRIMARY_EARNINGS_SESSIONS)]
...
def render_weekly_earnings_grid(
    ...
    for day in days:
        ...
        hidden_unknown = int((~day_rows["Session"].isin(PRIMARY_EARNINGS_SESSIONS)).sum()) if not day_rows.empty and "Session" in day_rows else 0
        if hidden_unknown:
...
def render_unconfirmed_earnings_section(frame: pd.DataFrame, show_table: bool) -> dict[str, int]:
    ...
    with st.expander(f"Earnings with Unconfirmed or Non-Standard Report Time ({len(unconfirmed):,})", expanded=bool(show_table)):
        render_section_title(
...
def filter_earnings_frame(
    ...
    if upcoming_only:
        event_dates = pd.to_datetime(filtered["Earnings Date"], errors="coerce").dt.date
        reported = filtered["Reported"].fillna(False) if "Reported" in filtered else False
        filtered = filtered[(~reported) & (event_dates >= eastern_now().date())]
...
def reset_earnings_filter_state() -> None:
...
def render_earnings_calendar_tab_legacy_disabled() -> None:
    ...
    if st.session_state.pop("earnings_reset_pending", False):
        reset_earnings_filter_state()
    for key, value in EARNINGS_FILTER_DEFAULTS.items():
        if key not in st.session_state:
            st.session_state[key] = list(value) if isinstance(value, list) else value
    if st.session_state.get("earnings_filter_schema") != 3:
        st.session_state["earnings_show_reported"] = False
        st.session_state["earnings_show_time_not_supplied"] = False
        if st.session_state.get("earnings_session_filter") == "Time Not Supplied":
            st.session_state["earnings_session_filter"] = "All"
        st.session_state["earnings_filter_schema"] = 3
    if st.session_state.get("earnings_session_filter") not in {"All", "Before Open", "After Close"}:
        st.session_state["earnings_session_filter"] = "All"
    if "earnings_week_start" not in st.session_state:
        st.session_state["earnings_week_start"] = week_monday()
    with st.container(border=True):
        nav_cols = st.columns([0.48, 1.1, 0.48, 0.9, 0.86, 0.68, 0.62], gap="small")
        with nav_cols[0]:
        ...
        with nav_cols[3]:
            session_filter = st.selectbox("Session", ["All", "Before Open", "After Close"], key="earnings_session_filter")
        with nav_cols[4]:
        ...
        with nav_cols[6]:
            reset_clicked = st.button("Reset", use_container_width=True, key="earnings_reset_filters")
        if reset_clicked:
            st.session_state["earnings_reset_pending"] = True
            st.rerun()
        if refresh_clicked:
        ...
        with filter_cols[1]:
            max_scan = st.slider("Scan size", 25, 200, 200, 25, key="earnings_scan_size")
        with filter_cols[2]:
        ...
        toggle_cols = st.columns([0.78, 0.68, 0.72, 1.05, 1.55], gap="small")
        with toggle_cols[0]:
        ...
        with toggle_cols[3]:
            show_time_not_supplied = True
            st.caption("Unconfirmed report times stay out of the grid and are listed below.")
        with toggle_cols[4]:
    ...
    if sector_options:
        st.session_state["earnings_sector_filter"] = [
            value for value in st.session_state.get("earnings_sector_filter", []) if value in sector_options
        ]
    ...
    if bucket_options:
        st.session_state["earnings_expanded_buckets"] = [
            value for value in st.session_state.get("earnings_expanded_buckets", []) if value in bucket_options
        ]
    expanded_buckets = (
        st.multiselect(
            "Show all bucket(s)",
            bucket_options,
            default=[],
            key="earnings_expanded_buckets",
            help="By default each day/session renders only the top Cards/session names. Select a bucket here to render its full list.",
        )
        if bucket_options
        else []
    )
    ...
    with st.expander("Expanded earnings detail", expanded=False):
        if filtered.empty:
            ...
            ticker_options = ["Select a ticker"] + filtered["Ticker"].dropna().astype(str).drop_duplicates().tolist()
            ...
            if selected_ticker == "Select a ticker":
                ...
                detail = filtered[filtered["Ticker"].astype(str).eq(selected_ticker)]
                detail = detail[[column for column in detail_cols if column in detail]].head(1).copy()
                render_earnings_detail_table(detail, height=table_height_for_rows(detail, max_height=180))
            if selected_ticker != "Select a ticker" and st.button("Open Company Analysis", key="earnings_open_company_analysis"):
    ...
    with st.expander("Earnings Data Validation", expanded=False):
        ...
        if not filtered.empty:
            if selected_card and selected_card != "Select a ticker":
                matched_sample = filtered[filtered["Ticker"].astype(str).eq(str(selected_card))]
                selected_sample = matched_sample.iloc[0] if not matched_sample.empty else filtered.iloc[0]
            else:
                selected_sample = filtered.iloc[0]
        ...
        for status_frame in status_frames:
            if status_frame.empty:
                continue
            for _, status_row in status_frame.iterrows():
                status_text = str(status_row.get("Status", status_row.get("status", ""))).lower()
                error_text = str(status_row.get("Error", status_row.get("error", "")))
                if "error" in status_text or "fail" in status_text or (error_text and error_text.lower() != "nan"):
                    label = status_row.get("Source", status_row.get("source", "source"))
        ...
        validation_rows = [
            ...
            {"Metric": "show_unconfirmed_table", "Value": str(bool(show_time_not_supplied))},
            ...
            {"Metric": "source_used", "Value": "Nasdaq/Yahoo earnings calendars + yfinance enrichment"},
            {"Metric": "exception_messages", "Value": "; ".join(exception_messages[:6]) if exception_messages else "None"},
            ...
            {"Metric": "rows_during_market", "Value": f"{int(filtered['Session'].eq('During Market').sum()) if not filtered.empty and 'Session' in filtered else 0:,}"},
            {"Metric": "rows_unconfirmed", "Value": f"{int(filtered['Session'].eq('Unconfirmed').sum()) if not filtered.empty and 'Session' in filtered else 0:,}"},
            {"Metric": "rows_reported", "Value": f"{int(filtered['Reported'].fillna(False).sum()) if not filtered.empty and 'Reported' in filtered else 0:,}"},
            {
                "Metric": "rows_upcoming",
                "Value": f"{int(((pd.to_datetime(filtered.get('Earnings Date', pd.Series(index=filtered.index, dtype=object)), errors='coerce').dt.date >= eastern_now().date()) & (~filtered.get('Reported', pd.Series(False, index=filtered.index)).fillna(False))).sum()) if not filtered.empty else 0:,}",
            },
            {"Metric": "number_of_eps_sign_warnings", "Value": f"{len([w for w in filtered.attrs.get('validation_warnings', []) if 'EPS' in w]):,}"},
            {"Metric": "number_of_revenue_source_warnings", "Value": f"{len([w for w in filtered.attrs.get('validation_warnings', []) if 'revenue' in w.lower()]):,}"},
            {"Metric": "number_of_session_warnings", "Value": f"{len([w for w in filtered.attrs.get('validation_warnings', []) if 'session' in w.lower()]):,}"},
            {"Metric": "first_20_validation_warnings", "Value": "; ".join(filtered.attrs.get('validation_warnings', [])[:20]) if filtered.attrs.get('validation_warnings') else "None"},
...
def render_earnings_calendar_tab() -> None:
    ...
    if "earnings_week_start" not in st.session_state:
        st.session_state["earnings_week_start"] = week_monday()
    if st.session_state.get("earnings_filter_schema") != 6:
        st.session_state["earnings_cards_per_session"] = 5
        st.session_state["earnings_show_reported"] = True
        st.session_state["earnings_show_time_not_supplied"] = False
        st.session_state["earnings_search_query"] = ""
        st.session_state["earnings_stocktwits_filter"] = False
        st.session_state["earnings_stocktwits_min_mentions"] = 0
        st.session_state["earnings_expanded_buckets"] = []
        st.session_state["earnings_filter_schema"] = 6

    with st.container(border=True):
        controls = st.columns([0.48, 1.02, 0.48, 1.45, 0.62, 0.85, 0.92], gap="small")
        with controls[0]:
        ...
        with controls[3]:
            search_query = st.text_input(
                ...
                placeholder="AMPX, IONQ, Palantir",
                key="earnings_search_query",
            )
        with controls[4]:
            refresh_clicked = st.button("Refresh", use_container_width=True, key="earnings_simple_refresh")
        with controls[5]:
            show_reported_results = st.toggle("Reported results", value=True, key="earnings_show_reported")
        with controls[6]:
            show_unconfirmed_times = st.toggle("Show unconfirmed times", value=False, key="earnings_show_time_not_supplied")
        filter_cols = st.columns([0.9, 0.75, 0.75, 2.2], gap="small")
        with filter_cols[0]:
            stocktwits_filter_enabled = st.toggle("Show most discussed on Stocktwits", value=False, key="earnings_stocktwits_filter")
        with filter_cols[1]:
            if stocktwits_filter_enabled:
                stocktwits_min_mentions = int(st.slider("Minimum Stocktwits mentions", 0, 100, int(st.session_state.get("earnings_stocktwits_min_mentions", 0)), key="earnings_stocktwits_min_mentions"))
            else:
                stocktwits_min_mentions = int(st.session_state.get("earnings_stocktwits_min_mentions", 0))
        with filter_cols[2]:
            if st.button("Clear search", use_container_width=True, key="earnings_clear_search"):
                st.session_state["earnings_search_query"] = ""
                st.rerun()
        with filter_cols[3]:
            st.caption("Main grid uses confirmed earnings date and session only. Stocktwits is optional and never builds the calendar.")

    week_end = week_start + timedelta(days=4)
    if refresh_clicked:
        load_scheduled_earnings_calendar.clear()
        load_weekly_earnings_calendar.clear()
        load_stocktwits_discussion.clear()
        load_latest_reported_quarterly_earnings.clear()
        load_latest_quarterly_earnings.clear()
        fetch_weekly_earnings_releases.clear()
        fetch_weekly_earnings_calendar_sources.clear()
    ...
    rows_after_week_filter = len(earnings_df)
    display_frame = earnings_df.copy()
    search_text = str(search_query or "").strip()
    if search_text and not display_frame.empty:
        search_mask = (
            display_frame.get("Ticker", pd.Series(index=display_frame.index, dtype=object)).astype(str).str.contains(re.escape(search_text), case=False, na=False)
            | display_frame.get("Company", pd.Series(index=display_frame.index, dtype=object)).astype(str).str.contains(re.escape(search_text), case=False, na=False)
        )
        display_frame = display_frame[search_mask].copy()
    rows_after_search_filter = len(display_frame)

    stocktwits_frame = pd.DataFrame()
    stocktwits_statuses: list[dict[str, object]] = []
    stocktwits_refreshed = None
    if stocktwits_filter_enabled and not display_frame.empty:
        with st.spinner("Checking Stocktwits discussion..."):
            stocktwits_frame, stocktwits_statuses, stocktwits_refreshed = load_stocktwits_discussion(
                tuple(display_frame["Ticker"].dropna().astype(str).drop_duplicates().tolist())
            )
        if isinstance(stocktwits_frame, pd.DataFrame) and not stocktwits_frame.empty:
            display_frame = display_frame.merge(
                stocktwits_frame.drop(columns=["ticker"], errors="ignore"),
                on="Ticker",
                how="left",
            )
            display_frame["stocktwits_mentions"] = pd.to_numeric(display_frame.get("stocktwits_mentions"), errors="coerce").fillna(0)
            display_frame = display_frame[display_frame["stocktwits_mentions"] >= stocktwits_min_mentions].copy()
        else:
            st.warning("Stocktwits data unavailable. Showing all earnings names instead.")
    rows_after_stocktwits_filter = len(display_frame)
    ...
    render_earnings_summary(display_frame, week_start, earnings_df)
    if search_text and display_frame.empty:
        st.info("No earnings found for this search in the selected week.")

    status_frame = pd.DataFrame(list(earnings_statuses) + list(visible_enrichment_statuses) + list(stocktwits_statuses))
    ...
    expanded_buckets = []
    bucket_options = earnings_bucket_options(display_frame, week_start, "All", False)
    if bucket_options:
        expanded_buckets = st.multiselect(
            "Show all bucket(s)",
            bucket_options,
            default=[item for item in st.session_state.get("earnings_expanded_buckets", []) if item in bucket_options],
            key="earnings_expanded_buckets",
        )

    confirmed = display_frame[display_frame["Session"].isin(PRIMARY_EARNINGS_SESSIONS)] if not display_frame.empty and "Session" in display_frame else pd.DataFrame()
    if confirmed.empty:
        st.info("No confirmed Before Open or After Close earnings releases were found for this week.")
        if not display_frame.empty and "Session" in display_frame and int((~display_frame["Session"].isin(PRIMARY_EARNINGS_SESSIONS)).sum()):
    ...
    placement_stats = earnings_grid_validation(display_frame, week_start, week_end)
    ...
    with st.expander("Expanded earnings detail", expanded=False):
        if display_frame.empty:
            ...
            ticker_options = ["Select a ticker"] + display_frame["Ticker"].dropna().astype(str).drop_duplicates().tolist()
            selected_ticker = st.selectbox("Earnings card", ticker_options, key="earnings_open_ticker")
            if selected_ticker == "Select a ticker":
                st.caption("Select an earnings card to view details.")
            else:
                ...
                detail = display_frame[display_frame["Ticker"].astype(str).eq(selected_ticker)]
                detail = detail[[column for column in detail_cols if column in detail]].head(1).copy()
    ...
    with st.expander("Earnings Data Validation", expanded=False):
        ...
        if not display_frame.empty:
            if selected_card and selected_card != "Select a ticker":
                matched_sample = display_frame[display_frame["Ticker"].astype(str).eq(str(selected_card))]
                selected_sample = matched_sample.iloc[0] if not matched_sample.empty else display_frame.iloc[0]
            else:
                selected_sample = display_frame.iloc[0]
        sample_metrics = earnings_card_sample_metrics(selected_sample)
        before_tickers = display_frame.loc[display_frame["Session"].eq("Before Open"), "Ticker"].dropna().astype(str).head(20).tolist() if not display_frame.empty and "Session" in display_frame else []
        after_tickers = display_frame.loc[display_frame["Session"].eq("After Close"), "Ticker"].dropna().astype(str).head(20).tolist() if not display_frame.empty and "Session" in display_frame else []
        unconfirmed_tickers = display_frame.loc[~display_frame["Session"].isin(PRIMARY_EARNINGS_SESSIONS), "Ticker"].dropna().astype(str).head(20).tolist() if not display_frame.empty and "Session" in display_frame else []
        reported_count = int(display_frame["Reported"].fillna(False).sum()) if not display_frame.empty and "Reported" in display_frame else 0
        pending_count = max(len(display_frame) - reported_count, 0)
        stocktwits_missing_count = 0
        if stocktwits_filter_enabled and not display_frame.empty:
            stocktwits_missing_count = int(pd.to_numeric(display_frame.get("stocktwits_mentions", pd.Series(index=display_frame.index, dtype=float)), errors="coerce").isna().sum())
        validation_warnings = []
        validation_warnings.extend(str(item) for item in earnings_df.attrs.get("validation_warnings", [])[:30] if item)
        validation_warnings.extend(str(item) for item in placement_stats.get("warnings", [])[:20] if item)
        validation_rows = [
            {"Metric": "selected_week_start", "Value": str(week_start)},
            {"Metric": "selected_week_end", "Value": str(week_end)},
            {"Metric": "search_query", "Value": search_text or ""},
            {"Metric": "stocktwits_filter_enabled", "Value": str(bool(stocktwits_filter_enabled))},
            {"Metric": "stocktwits_min_mentions", "Value": str(stocktwits_min_mentions)},
            {"Metric": "total_rows_loaded", "Value": f"{len(earnings_df):,}"},
            {"Metric": "rows_after_week_filter", "Value": f"{rows_after_week_filter:,}"},
            {"Metric": "rows_after_search_filter", "Value": f"{rows_after_search_filter:,}"},
            {"Metric": "rows_after_stocktwits_filter", "Value": f"{rows_after_stocktwits_filter:,}"},
            {"Metric": "confirmed_before_open_count", "Value": f"{int(display_frame['Session'].eq('Before Open').sum()) if not display_frame.empty and 'Session' in display_frame else 0:,}"},
            {"Metric": "confirmed_after_close_count", "Value": f"{int(display_frame['Session'].eq('After Close').sum()) if not display_frame.empty and 'Session' in display_frame else 0:,}"},
            {"Metric": "during_market_count", "Value": f"{int(display_frame['Session'].eq('During Market').sum()) if not display_frame.empty and 'Session' in display_frame else 0:,}"},
            {"Metric": "unconfirmed_count", "Value": f"{int(display_frame['Session'].eq('Unconfirmed').sum()) if not display_frame.empty and 'Session' in display_frame else 0:,}"},
            ...
            {"Metric": "stocktwits_rows_loaded", "Value": f"{len(stocktwits_frame) if isinstance(stocktwits_frame, pd.DataFrame) else 0:,}"},
            {"Metric": "stocktwits_missing_count", "Value": f"{stocktwits_missing_count:,}"},
            {"Metric": "stocktwits_source", "Value": "Stocktwits API" if stocktwits_filter_enabled else "Disabled"},
            {"Metric": "stocktwits_last_updated", "Value": stocktwits_refreshed.strftime("%I:%M:%S %p ET").lstrip("0") if isinstance(stocktwits_refreshed, datetime) else "N/A"},
            ...
            {"Metric": "validation_warnings", "Value": "; ".join(validation_warnings[:50]) if validation_warnings else "None"},
            {"Metric": "selected_card", "Value": selected_card or "N/A"},
            {"Metric": "selected_ticker_card_sample", "Value": sample_metrics.get("selected_ticker_card_sample", "N/A")},
            {"Metric": "selected_card_eps_estimate", "Value": sample_metrics.get("selected_card_eps_estimate", "N/A")},
            {"Metric": "selected_card_actual_eps", "Value": sample_metrics.get("selected_card_actual_eps", "N/A")},
        ...
        render_dashboard_table(pd.DataFrame(validation_rows), height=420)
        if not display_frame.empty:
            validation_sample = display_frame.copy()
            validation_sample["bucket_rendered"] = validation_sample.apply(
                lambda row: normalize_earnings_session(row.get("Session"), row.get("Earnings DateTime"))
                if parse_date(row.get("Earnings Date")) and week_start <= parse_date(row.get("Earnings Date")) <= week_end and normalize_earnings_session(row.get("Session"), row.get("Earnings DateTime")) in PRIMARY_EARNINGS_SESSIONS
                else "Excluded from main grid",
    ...
    with st.expander("Global Earnings Data Validation", expanded=False):
        status_counts = earnings_df.get("validation_status", pd.Series(dtype=object)).value_counts(dropna=False).to_dict() if not earnings_df.empty else {}
        warning_series = earnings_df.get("validation_warnings", pd.Series(dtype=object)).dropna().astype(str) if not earnings_df.empty else pd.Series(dtype=object)
        global_rows = [
            {"Metric": "total_scheduled_earnings_rows", "Value": f"{len(earnings_df):,}"},
            {"Metric": "total_latest_reported_rows", "Value": "Loaded per ticker in Company Analysis"},
            {"Metric": "rows_valid", "Value": f"{int(status_counts.get('Valid', 0)):,}"},
            {"Metric": "rows_partial", "Value": f"{int(status_counts.get('Partial', 0)):,}"},
            {"Metric": "rows_invalid", "Value": f"{int(status_counts.get('Invalid', 0)):,}"},
            {"Metric": "rows_with_quarter_mismatch", "Value": f"{int(warning_series.str.contains('quarter mismatch', case=False, na=False).sum()) if not warning_series.empty else 0:,}"},
            {"Metric": "rows_with_missing_actuals", "Value": f"{int((pd.to_numeric(earnings_df.get('Actual EPS', pd.Series(index=earnings_df.index, dtype=float)), errors='coerce').isna() & pd.to_numeric(earnings_df.get('Actual Revenue', pd.Series(index=earnings_df.index, dtype=float)), errors='coerce').isna()).sum()) if not earnings_df.empty else 0:,}"},
            {"Metric": "rows_with_missing_estimates", "Value": f"{int((pd.to_numeric(earnings_df.get('EPS Estimate', pd.Series(index=earnings_df.index, dtype=float)), errors='coerce').isna() & pd.to_numeric(earnings_df.get('Revenue Estimate', pd.Series(index=earnings_df.index, dtype=float)), errors='coerce').isna()).sum()) if not earnings_df.empty else 0:,}"},
            {"Metric": "rows_with_eps_sign_warning", "Value": f"{int(warning_series.str.contains('EPS display lost sign', case=False, na=False).sum()) if not warning_series.empty else 0:,}"},
            {"Metric": "rows_with_revenue_unit_warning", "Value": f"{int(warning_series.str.contains('revenue unit', case=False, na=False).sum()) if not warning_series.empty else 0:,}"},
            {"Metric": "rows_with_session_warning", "Value": f"{int(warning_series.str.contains('session', case=False, na=False).sum()) if not warning_series.empty else 0:,}"},
            {"Metric": "rows_excluded_due_to_future_scheduled_date", "Value": "N/A for scheduled calendar"},
            {"Metric": "rows_excluded_due_to_annual_or_ttm_data", "Value": "N/A for scheduled calendar"},
            {"Metric": "first_50_validation_warnings", "Value": "; ".join(warning_series[warning_series.ne('')].head(50).tolist()) or "None"},
        ]
        render_dashboard_table(pd.DataFrame(global_rows), height=360)
        if not earnings_df.empty:
            sample_cols = [
                "ticker",
                "company_name",
                ...
                "revenue_surprise",
                "revenue_surprise_pct",
                "source_earnings_date",
                "source_actuals",
                "source_estimates",
                "validation_status",
                "validation_warnings",
            ]
            render_dashboard_table(earnings_df[[column for column in sample_cols if column in earnings_df]].head(50), height=320)
